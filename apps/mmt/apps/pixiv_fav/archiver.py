...
from mmt.agents import agent
from mmt.agents.pixiv import PixivAgent, AgentBase
from .model import BookmarkWork, FollowWork, initialize_database

confctl.util.default_arg_config_loggers()
litter.connect(app_name="pixiv_fav_archiver")

initialize_database(config.get("db_url"))

class PixivFavArchiver:
    papi: PixivAgent | None = PixivAgent.api()
...
@agent("mmt.apps.pixiv_fav_archiver")
class PixivFavArchiverAgent(PixivFavArchiver, AgentBase):
    pass
