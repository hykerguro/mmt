        ...
        try:
            ...
            if ret_data["error"]:
                _lp("pixiv_webapi.request.error",
                    {"method": method, "url": url, "kwargs": kwargs, "ret_data": ret_data})
                raise PixivWebAPIException(ret_data["message"])
            _lp("pixiv_webapi.request.success", {"method": method, "url": url, "kwargs": kwargs, "ret_data": ret_data})
            return ret_data["body"]
        except requests.exceptions.RequestException as e:
            logger.error(f"请求失败: {e}")
            _lp("pixiv_webapi.request.fail", {"method": method, "url": url, "kwargs": kwargs, "error": e})
            raise e
